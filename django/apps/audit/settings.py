...
from apps.audit.models import CRUDEvent, LoginEvent, RequestEvent
...
WATCH_AUTH_EVENTS = getattr(
    settings, 'AUDIT_WATCH_AUTH_EVENTS', True)
WATCH_MODEL_EVENTS = getattr(
    settings, 'AUDIT_WATCH_MODEL_EVENTS', True)
WATCH_REQUEST_EVENTS = getattr(
    settings, 'AUDIT_WATCH_REQUEST_EVENTS', True)
REMOTE_ADDR_HEADER = getattr(
    settings, 'AUDIT_REMOTE_ADDR_HEADER', 'REMOTE_ADDR')

USER_DB_CONSTRAINT = bool(
    getattr(settings, 'AUDIT_USER_DB_CONSTRAINT', True))

# logging backend settings
LOGGING_BACKEND = getattr(
    settings, 'AUDIT_LOGGING_BACKEND', 'apps.audit.backends.ModelBackend')
...
UNREGISTERED_CLASSES = [CRUDEvent, LoginEvent, RequestEvent, Migration,
                        Session, Permission, ContentType, MigrationRecorder.Migration]
...
if apps.is_installed('silk'):
    from silk.models import Request, Response, SQLQuery, BaseProfile
    UNREGISTERED_CLASSES += [Request, Response, SQLQuery, BaseProfile]

if apps.is_installed('django_celery_results'):
    from django_celery_results.models import TaskResult
    UNREGISTERED_CLASSES += [TaskResult]
...
UNREGISTERED_CLASSES = getattr(
    settings, 'AUDIT_UNREGISTERED_CLASSES_DEFAULT', UNREGISTERED_CLASSES)
UNREGISTERED_CLASSES.extend(
    getattr(settings, 'AUDIT_UNREGISTERED_CLASSES_EXTRA', []))
...
REGISTERED_CLASSES = getattr(
    settings, 'AUDIT_REGISTERED_CLASSES', [])
...
UNREGISTERED_URLS.extend(
    getattr(settings, 'AUDIT_UNREGISTERED_URLS_EXTRA', []))
...
ADMIN_SHOW_MODEL_EVENTS = getattr(
    settings, 'AUDIT_ADMIN_SHOW_MODEL_EVENTS', True)
ADMIN_SHOW_AUTH_EVENTS = getattr(
    settings, 'AUDIT_ADMIN_SHOW_AUTH_EVENTS', True)
ADMIN_SHOW_REQUEST_EVENTS = getattr(
    settings, 'AUDIT_ADMIN_SHOW_REQUEST_EVENTS', True)


# project defined callbacks
CRUD_DIFFERENCE_CALLBACKS = []
CRUD_DIFFERENCE_CALLBACKS = getattr(
    settings, 'AUDIT_CRUD_DIFFERENCE_CALLBACKS', CRUD_DIFFERENCE_CALLBACKS)
DATABASE_ALIAS = getattr(
    settings, 'AUDIT_DATABASE_ALIAS', django.db.utils.DEFAULT_DB_ALIAS)
...
CRUD_EVENT_NO_CHANGED_FIELDS_SKIP = getattr(
    settings, "AUDIT_CRUD_EVENT_NO_CHANGED_FIELDS_SKIP", True)
...
#   AUDIT_TRUNCATE_TABLE_SQL_STATEMENT = 'TRUNCATE TABLE "{db_table}"'  # for Postgresql
...
TRUNCATE_TABLE_SQL_STATEMENT = getattr(
    settings, 'AUDIT_TRUNCATE_TABLE_SQL_STATEMENT', '')

# Changeview filters configuration
CRUD_EVENT_LIST_FILTER = getattr(settings, 'AUDIT_CRUD_EVENT_LIST_FILTER', [
                                 'event_type', 'content_type', 'datetime','user' ])
LOGIN_EVENT_LIST_FILTER = getattr(
    settings, 'AUDIT_LOGIN_EVENT_LIST_FILTER', ['login_type', 'datetime', 'user'])
REQUEST_EVENT_LIST_FILTER = getattr(
    settings, 'AUDIT_REQUEST_EVENT_LIST_FILTER', ['method', 'datetime','user' ])

# Search fields configuration
CRUD_EVENT_SEARCH_FIELDS = getattr(settings, 'AUDIT_CRUD_EVENT_SEARCH_FIELDS', [
                                   '=object_id', 'object_json_repr', ])
LOGIN_EVENT_SEARCH_FIELDS = getattr(
    settings, 'AUDIT_LOGIN_EVENT_SEARCH_FIELDS', ['=remote_ip', 'username', ])
REQUEST_EVENT_SEARCH_FIELDS = getattr(settings, 'AUDIT_REQUEST_EVENT_SEARCH_FIELDS', [
                                      '=remote_ip', 'user__username', 'url', 'query_string', ])

READONLY_EVENTS = getattr(settings, 'AUDIT_READONLY_EVENTS', False)

IGNORE_FIELDS = getattr(settings, 'AUDIT_IGNORE_FIELDS', 
['author', 'editor', 'date_add','date_edit','organization','author_display_name', 'editor_display_name'])
