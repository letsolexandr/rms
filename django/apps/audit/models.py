import imp
from statistics import mode
from django.conf import settings
from django.contrib.contenttypes.models import ContentType
from django.db import models
from django.utils.translation import gettext_lazy as _
from cacheops import invalidate_obj


class CRUDEventTypes(models.IntegerChoices):
    CREATE = 1, 'Створення'
    UPDATE = 2, 'Зміна'
    DELETE = 3, 'Видалення'
    M2M_CHANGE = 4, 'Багато до багатьох (зміна)'
    M2M_CHANGE_REV = 5, 'Багато до багатьох (зміна)(Reverse)'
    M2M_ADD = 6, 'Багато до багатьох (додавання)'
    M2M_ADD_REV = 7, 'Багато до багатьох (додавання)(Reverse)'
    M2M_REMOVE = 8, 'Багато до багатьох (видалення)'
    M2M_REMOVE_REV = 9, 'Багато до багатьох (видалення)(Reverse)'
    TRY_DELETE = 10, 'Спроба видалення'


class CRUDEvent(models.Model):
    CREATE = 1
    UPDATE = 2
    DELETE = 3
    M2M_CHANGE = 4
    M2M_CHANGE_REV = 5
    M2M_ADD = 6
    M2M_ADD_REV = 7
    M2M_REMOVE = 8
    M2M_REMOVE_REV = 9
    TRY_DELETE = 10

    TYPES = (
        (CREATE, _('Create')),
        (UPDATE, _('Update')),
        (DELETE, _('Delete')),
        (M2M_CHANGE, _('Many-to-Many Change')),
        (M2M_CHANGE_REV, _('Reverse Many-to-Many Change')),
        (M2M_ADD, _('Many-to-Many Add')),
        (M2M_ADD_REV, _('Reverse Many-to-Many Add')),
        (M2M_REMOVE, _('Many-to-Many Remove')),
        (M2M_REMOVE_REV, _('Reverse Many-to-Many Remove')),
        # TODO Українська локалізація
        (TRY_DELETE, 'Спроба видалення'),
    )

    event_type = models.SmallIntegerField(
        choices=CRUDEventTypes.choices, verbose_name=_('Event type'),db_index=True)
    object_id = models.CharField(max_length=255, verbose_name=_('Object ID'),db_index=True)
    content_type = models.ForeignKey(
        ContentType, on_delete=models.CASCADE, db_constraint=False, verbose_name=_('Content type'))
    object_repr = models.TextField(
        null=True, blank=True, verbose_name=_('Object representation'))
    object_json_repr = models.TextField(
        null=True, blank=True, verbose_name=_('Object JSON representation'))
    changed_fields = models.JSONField(
        null=True, blank=True, verbose_name=_('Changed fields'))
    user = models.ForeignKey(settings.AUTH_USER_MODEL, null=True,
                             blank=True, on_delete=models.SET_NULL,
                             db_constraint=False, verbose_name=_('User'))
    user_data = models.JSONField(null=True, editable=False)
    hr_object_repr = models.JSONField(null=True, editable=False)
    user_pk_as_string = models.CharField(max_length=255, null=True, blank=True,
                                         help_text=_('String version of the user pk'), verbose_name=_('User PK as string'))
    user_as_string = models.CharField(max_length=255, null=True, blank=True,
                                      help_text=_('String version of the user'), verbose_name=_('User as string'))
    datetime = models.DateTimeField(
        auto_now_add=True, verbose_name=_('Date time'),db_index=True)
    change_comment = models.CharField(
        max_length=250, null=True, verbose_name="Коментар")

    def save(self, *args, **kwargs):
        self.set_user_data()
        res = super().save(*args, **kwargs)
        self.set_hr_data()
        return res

    def set_user_data(self):
        from apps.audit.serializers import HistoryUserSerializer
        if self.user:
            ser = HistoryUserSerializer(self.user)
            data = ser.data
            self.user_data = data

    def set_hr_data(self):
        from apps.audit.settings import IGNORE_FIELDS
        from apps.audit.utils import get_field_value, smart_text
        hr_data = {}
        model = self.content_type.model_class()
        fields = model._meta.fields
        if not hasattr(model, 'objects_with_deleted'):
            return
        inst = model.objects_with_deleted.get(pk=self.object_id)

        for field in fields:
            if field.name in IGNORE_FIELDS:
                continue
            value = get_field_value(inst, field)
            if not value or value == 'None':
                continue
            hr_data[field.name] = {"verbose_name": field.verbose_name.title().lower(),
                                   "value": smart_text(value),
                                   "type": field.__class__.__name__}

        self._meta.model.objects.filter(
            pk=self.pk).update(hr_object_repr=hr_data)
        invalidate_obj(self)

    def is_create(self):
        return self.CREATE == self.event_type

    def is_update(self):
        return self.UPDATE == self.event_type

    def is_delete(self):
        return self.DELETE == self.event_type

    def __str__(self) -> str:
        return f'{self.user_as_string}: {self.object_repr}: {self.get_event_type_display()}'


    class Meta:
        verbose_name = _('CRUD event')
        verbose_name_plural = _('CRUD events')
        ordering = ['-datetime']
        index_together = ['object_id', 'content_type', ]
        default_permissions = ('delete', 'view')


class LoginEvent(models.Model):
    LOGIN = 0
    LOGOUT = 1
    FAILED = 2
    TYPES = (
        (LOGIN, _('Login')),
        (LOGOUT, _('Logout')),
        (FAILED, _('Failed login')),
    )
    login_type = models.SmallIntegerField(
        choices=TYPES, verbose_name=_('Event type'))
    username = models.CharField(
        max_length=255, null=True, blank=True, verbose_name=_('Username'))
    user = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True,
                             on_delete=models.SET_NULL, db_constraint=False,
                             verbose_name=_('User'))
    remote_ip = models.CharField(
        max_length=50, null=True, db_index=True, verbose_name=_('Remote IP'))
    datetime = models.DateTimeField(
        auto_now_add=True, verbose_name=_('Date time'))
    user_as_string = models.CharField(max_length=255, null=True, blank=True,
                                      help_text=_('String version of the user'), verbose_name=_('User as string'))
    comment = models.CharField(
        max_length=300, null=True, verbose_name="Коментар")                                  

    class Meta:
        verbose_name = _('login event')
        verbose_name_plural = _('login events')
        ordering = ['-datetime']


class RequestEvent(models.Model):
    url = models.CharField(null=False, db_index=True,
                           max_length=254, verbose_name=_('URL'))
    method = models.CharField(
        max_length=20, null=False, db_index=True, verbose_name=_('Method'))
    query_string = models.TextField(null=True, verbose_name=_('Query string'))
    user = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True,
                             on_delete=models.SET_NULL, db_constraint=False,
                             verbose_name=_('User'))
    remote_ip = models.CharField(
        max_length=50, null=True, db_index=True, verbose_name=_('Remote IP'))
    datetime = models.DateTimeField(
        auto_now_add=True, verbose_name=_('Date time'))

    class Meta:
        verbose_name = _('request event')
        verbose_name_plural = _('request events')
        ordering = ['-datetime']
        default_permissions = ('delete', 'view')
