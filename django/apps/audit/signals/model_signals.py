...
from apps.audit.middleware.easyaudit import get_current_request, \
    get_current_user
from apps.audit.models import CRUDEvent
from apps.audit.settings import REGISTERED_CLASSES, UNREGISTERED_CLASSES, \
    WATCH_MODEL_EVENTS, CRUD_DIFFERENCE_CALLBACKS, LOGGING_BACKEND, \
    DATABASE_ALIAS
from apps.audit.utils import get_m2m_field_name, model_delta, get_m2m_field
    ...
    try:
        with transaction.atomic(using=using):
            ...
            if not created:
                ...
                enable_skip_nochanged = getattr(settings, "AUDIT_CRUD_EVENT_NO_CHANGED_FIELDS_SKIP", False)
                #TODO не підтягуються налаштування, пофіксити
                # if not delta and enable_skip_nochanged:
                if not delta:
                    return False
                    ...
                    try:
                        ...
                        with transaction.atomic(using=DATABASE_ALIAS):
                            crud_event = audit_logger.crud({
                                ...
                                'change_comment':instance._change_reason if hasattr(instance,'_change_reason') else None,
                                'object_json_repr': object_json_repr,
                                'changed_fields': changed_fields,
                                'content_type_id': c_t.id,
                                'object_id': instance.pk,
                                'user_id': getattr(user, 'id', None),
                                'user_as_string': getattr(user, 'display_name', None),
...
def m2m_changed(sender, instance, action, reverse, model, pk_set, using, **kwargs):
    ...
    try:
        with transaction.atomic(using=using):
            ...
            def crud_flow():
                try:
                    field = get_m2m_field(model, instance)
                    changed_fields = {field.name: list(pk_set), 'pk_set': list(
                        pk_set), 'verbose_name': field.verbose_name.title().lower()}
                    with transaction.atomic(using=DATABASE_ALIAS):
                        crud_event = audit_logger.crud({
                            'event_type': event_type,
                            'object_repr': str(instance),
